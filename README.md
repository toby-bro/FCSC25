# FCSC 2025 - Writeups

Here are some writeups for the challenges I solved during the FCSC 2025 CTF. The writeups are organized in the category in which the challenges were.

Some writeups are in french, when the problem was too mathematized and I new my english would hinder me.

My two most interesting writeups this year are (both are in french btw):

- [Le calme avant la tempest](./hardware/Le_calme_avant_la_tempest/)
- [Un cercle est un carré](./misc/Un_cercle_est_un_carre/)

followed but much further away by:

- [Problèmeuh](./crypto/Problemeuh/)
- [babyfuscation](./reverse/babyfuscation/)

## Challenges

These are all the challenges I wrote writeups for.

- [crypto](./crypto/)
  - [CocoRiCo](./crypto/CocoRiCo/)
  - [La quête de l'anneau](./crypto/La_quete_de_l_anneau/)
  - [Problèmeuh](./crypto/Problemeuh/)
  - [RSA-WTF](./crypto/RSA_WTF/)
- [hardware](./hardware/)
  - [Binary Lullaby](./hardware/Binary_Lullaby/)
  - [Grand Classic Hotel](./hardware/Grand_classic_hotel/)
  - [Le calme avant la tempest](./hardware/Le_calme_avant_la_tempest/)
  - [Mechanical Display](./hardware/Mechanical_Display/)
  - [Signal sur chat](./hardware/Signal_sur_chat/)
- [sca-faults](./sca-faults/)
  - [CryptoBro en détresse](./sca-faults/CryptoBro_en_detresse/)
- [reverse](./reverse/)
  - [babyfuscation](./reverse/babyfuscation/)
- [misc](./misc/)
  - [Il était une bergère](./misc/Il_etait_une_bergere/)
  - [Un cercle est un carré](./misc/Un_cercle_est_un_carre/)

## Attestation de participation

This year they gave out a certificate of score I guess I can leave it here

![Certificate](attestation-participation.png)
